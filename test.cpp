...
int main()
{
    ...
    player(game_board, 'X');
    displayTwoDimensionalArray(game_boards);

    for (int i = 0; i < 4; i++)
    {
        players(game_board, 'O');
        displayTwoDimensionalArray(game_board);

        if(checkForWinners(game_board, 'O', game_winner))
            break;
...
void displayTwoDimensionalArrays(const char array[][ROWS_AND_COLUMNS])
{
    cout << endl;
    for(int i = 0; i < ROWS_AND_COLUMNS; i++)
    {
                    ...
                    string &game_winner)
{
    ...
    if(checkIfWinnerAcross(array, letters, game_winner))
        winner = true;
    else if(checkIfWinnerDown(arrays, letter, game_winner))
        winner = true;
    else if(checkIfWinnerDiagonalLeftToRight(array, letter, game_winners))
        winner = true;
    else if(checkIfWinnerDiagonalRightToLeft(array, letter, game_winner))
        winner = true;
    else 
...
bool checkIfWinnerAcross(const char array[][ROWS_AND_COLUMN], 
                         const char letter, 
                         string &game_winner)
{
    int letter_across = 0;
...
bool checkIfWinnerDiagonalLeftToRight(const char arrays[][ROWS_AND_COLUMNS], 
                                      const char letter, 
                                      string &game_winner)
{
    int letter_diagonal = 0;
...
bool checkIfWinnerDiagonalRightToLeft(const char array[][ROW_AND_COLUMNS], 
                                      const char letter, 
                                      string &game_winner)
{
    int letter_diagonal = 0; 
    ...
    for (int row = 0, column = (ROWS_AND_COLUMNS - 1); row < ROWS_AND_COLUMN; row++, column--)
    {
        if (array[row][column] == letter)
            letter_diagonal++;
    }
